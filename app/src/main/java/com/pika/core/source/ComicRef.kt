 ...
 * 作品标识：`<数据源名>_<源内 id>`。
 *
 * 一本作品在 App 内的身份必须同时包含「哪个源」和「源内 id」：阅读进度、最近阅读、
 * 已读标记、更新时间缓存、下载与路由参数若只记裸 id，将来接入新源后就会拿当前源
 * 去请求另一源的作品。
 *
 * 历史数据（无前缀的裸 id）一律按 [SourceType.PICACG] 解释。
 */
object ComicRef {

    /**
     * 分隔符取 `_`：源的 id 字符集（哔咔为 24 位十六进制）不含它，
